...
const authRoutes = async (fastify, opts) => {
    ...
    fastify.post('/login', { schema: loginSchema }, async (request, reply) => {
        ...
        const token = jwt.sign({ id: user.id, email: user.email, roles: user.roles.map(r => r.nombre) }, process.env.JWT_SECRET || 'secret_key_change_me', { expiresIn: '24h' });
        return {
            token,
            user: {
                id: user.id,
    ...
    fastify.post('/register', {
        ...
        const token = jwt.sign({ id: usage.id, email: usage.email, roles: usage.roles.map(r => r.nombre) }, process.env.JWT_SECRET || 'secret_key_change_me', { expiresIn: '24h' });
